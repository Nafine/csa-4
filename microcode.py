...
MROM[11] = encode_signals(acc_l=1, alu_left=AluLeft.ACC, alu_right=AluRight.DR, cond=Cond.ALWAYS,
                          next_addr=0)  # ADD: ACC <- ACC + DR; -> FETCH

# SUB: ACC <- ACC - Mem[arg]
MROM[12] = encode_signals(ar_l=1, ar_sel=ArSel.CR_ARG)  # AR <- CR.arg
MROM[13] = encode_signals(dr_l=1)  # DR <- Mem[AR]
MROM[14] = encode_signals(acc_l=1, alu_left=AluLeft.ACC, alu_right=AluRight.DR, alu=Alu.SUB, cond=Cond.ALWAYS,
                          next_addr=0)  # SUB: ACC <- ACC - DR; -> FETCH

# MUL: ACC <- ACC * Mem[arg]
MROM[15] = encode_signals(ar_l=1, ar_sel=ArSel.CR_ARG)  # AR <- CR.arg
MROM[16] = encode_signals(dr_l=1)  # DR <- Mem[AR]
MROM[17] = encode_signals(acc_l=1, alu_left=AluLeft.ACC, alu_right=AluRight.DR, alu=Alu.MUL, cond=Cond.ALWAYS,
                          next_addr=0)  # MUL: ACC <- ACC * DR; -> FETCH

# DIV: ACC <- ACC / Mem[arg]
MROM[18] = encode_signals(ar_l=1, ar_sel=ArSel.CR_ARG)  # AR <- CR.arg
MROM[19] = encode_signals(dr_l=1)  # DR <- Mem[AR]
MROM[20] = encode_signals(acc_l=1, alu_left=AluLeft.ACC, alu_right=AluRight.DR, alu=Alu.DIV, cond=Cond.ALWAYS,
                          next_addr=0)  # DIV: ACC <- ACC // DR; -> FETCH

# REM: ACC <- ACC % Mem[arg]
MROM[21] = encode_signals(ar_l=1, ar_sel=ArSel.CR_ARG)  # AR <- CR.arg
MROM[22] = encode_signals(dr_l=1)  # DR <- Mem[AR]
MROM[23] = encode_signals(acc_l=1, alu_left=AluLeft.ACC, alu_right=AluRight.DR, alu=Alu.REM, cond=Cond.ALWAYS,
                          next_addr=0)  # REM: ACC <- ACC * DR; -> FETCH

# CMP: N,Z <- ACC - Mem[arg]
MROM[24] = encode_signals(ar_l=1, ar_sel=ArSel.CR_ARG)  # AR <- CR.arg
MROM[25] = encode_signals(dr_l=1)  # DR <- Mem[AR]
MROM[26] = encode_signals(alu_left=AluLeft.ACC, alu_right=AluRight.DR, alu=Alu.SUB, cond=Cond.ALWAYS,
