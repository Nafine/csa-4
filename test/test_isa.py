...
def test_load():
    ...
    print()
    for _ in range(5):  # FETCH 2 + LOAD 3
        cu.step()
        print(format_state(cu.snapshot()))
    assert cu.dp.acc == 0xCAFE, f'ACC={cu.dp.acc:#x}'
    assert cu.dp.ar == 0x10, f'AR={cu.dp.ar:#x}'
    assert cu.mp == 1, f'MP={cu.mp}'


def test_ldi():
    cu = ControlUnit()
    cu.dp.data_mem[0] = 0x03000040  # LDI #0x40
    for _ in range(3):  # FETCH 2 + LDI 1
        cu.step()
...
def test_store():
    ...
    cu.dp.acc = 0xBEEF
    for _ in range(4):  # FETCH 2 + STORE 2
        cu.step()
...
def test_add():
    ...
    cu.dp.acc = 0
    for _ in range(5):  # FETCH 2 + ADD 3
        cu.step()
...
def test_sub():
    cu = ControlUnit()
    cu.dp.data_mem[0] = 0x12000020  # SUB 0x20
    cu.dp.data_mem[0x20] = 15
    cu.dp.acc = 0
    for _ in range(5):  # FETCH 2 + SUB 3
        cu.step()
...
def test_mul():
    cu = ControlUnit()
    cu.dp.data_mem[0] = 0x14000020  # MUL 0x20
    cu.dp.data_mem[0x20] = 45
    cu.dp.acc = 3
    print()
    for _ in range(5):  # FETCH 2 + MUL 3
        cu.step()
        print(format_state(cu.snapshot()))
...
def test_div():
    cu = ControlUnit()
    cu.dp.data_mem[0] = 0x16000020  # DIV 0x20
    cu.dp.data_mem[0x20] = 4
    cu.dp.acc = 20
    for _ in range(5):  # FETCH 2 + DIV 3
        cu.step()
...
def test_rem():
    cu = ControlUnit()
    cu.dp.data_mem[0] = 0x18000020  # REM 0x20
    cu.dp.data_mem[0x20] = 3
    cu.dp.acc = 20
    for _ in range(5):  # FETCH 2 + REM 3
        cu.step()
